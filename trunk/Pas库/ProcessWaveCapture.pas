...
function InjectTargetPid(TargetPid: Cardinal): Boolean;
function InjectTargetExePath(ExePath: string): Cardinal;
...
//inject target
function InjectTargetPid(TargetPid: DWORD): Boolean;
var
  PHandle : THandle ;
  ipcnameData : string;
begin
...
//create process and inject dll
  ...
  si.cb:=SizeOf(si);
//  CreateProcess (PChar(ExePath),nil,nil,nil,False,0,nil,nil,si,pi);
  if CreateProcessExA(PChar(ExePath),nil,nil,nil,False,0,nil,nil,si,pi, 'AudioInterception.dll') then
  begin
...
//create ipc
  if Result <>0 then
  begin
    ipcnameData := 'processwavecapture_data_' + inttostr(Result);
    //创建IPC
    if not CreateIpcQueue(PAnsiChar(ipcnameData), GetAudioDataFromDLL)
    then Result:= 0;
  end;
end;
//Uninject target
